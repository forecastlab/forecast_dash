    ...
    def fit(self, y):
        ...
        r_forecast_dict = dict(
            self.forecast_lib.forecast(
                self.fit_results, h=1, level=robjects.IntVector(levels)
            ).items()
        )
...
def forecast_to_df(forecast_dict, first_value, first_time, levels):
...
with open("data_sources.json") as data_sources_json_file:
    ...
    for data_source_dict in data_sources_list:
        ...
        best_model.fit(series_df["value"])
        ...
        forecast_df = forecast_to_df(
            forecast_dict, first_value, first_time, levels=levels
        )
