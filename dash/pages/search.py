...
def add_dropdown_search_options():
    ...
    all_tags = [{"label": html.Div([html.Div(className="fa fa-hashtag"),html.Div(tag, style={'font-size': 15, 'padding-left': 10})], style={'display': 'flex', 'align-items': 'center', 'justify-content': 'center'}), "value": tag} for tag in sorted(set(all_tags))]

    # Load methods
    stats = get_forecast_data("statistics")
    all_methods = [
    {"label": html.Div([html.Div(className="fa fa-wrench"),html.Div(f"Winning Method - {method}", style={'font-size': 15, 'padding-left': 10})], style={'display': 'flex', 'align-items': 'center', 'justify-content': 'center'}), "value": method}
    ...
    all_titles = [    {"label": html.Div([html.Div(className="fa fa-globe"),html.Div(title, style={'font-size': 15, 'padding-left': 10})], style={'display': 'flex', 'align-items': 'center', 'justify-content': 'center'}), "value": title}
 for title in all_titles]

    all_options = all_tags + all_methods + all_titles
    all_options = sorted(all_options, key=lambda d: d["value"]) # previous was 'label', can't order by html.Div
